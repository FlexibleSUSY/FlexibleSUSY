...
(**
  Functions to perform an uncertainty estimate of NUHMSSMNoFVHimalayaEFTHiggs.
 *)

CalcNUHMSSMNoFVHimalayaEFTHiggsDMh::usage="\
The function takes the parameter point as input (same syntax as
FSNUHMSSMNoFVHimalayaEFTHiggsOpenHandle[]) and returns the 2-component list { Mh, DMh }
...
CalcNUHMSSMNoFVHimalayaEFTHiggsMh[a___, (fsSettings | fsSMParameters | fsModelParameters) -> s_List, r___] :=
    CalcNUHMSSMNoFVHimalayaEFTHiggsMh[a, Sequence @@ s, r];

CalcNUHMSSMNoFVHimalayaEFTHiggsMh[ytLoops_?NumericQ, Qpole_?NumericQ, Qm_?NumericQ, args__] :=
    Module[{handle, spec, tc},
           tc = thresholdCorrections /. { args };
           tc = If[IntegerQ[tc], tc,
                   thresholdCorrections /. Options[FSNUHMSSMNoFVHimalayaEFTHiggsOpenHandle]];
           If[ytLoops >= 0, tc = SetDigit[tc, 6, ytLoops]];
           handle = FSNUHMSSMNoFVHimalayaEFTHiggsOpenHandle[args];
           FSNUHMSSMNoFVHimalayaEFTHiggsSet[handle,
               fsSettings -> {
           ...
           spec = FSNUHMSSMNoFVHimalayaEFTHiggsCalculateSpectrum[handle];
           FSNUHMSSMNoFVHimalayaEFTHiggsCloseHandle[handle];
           If[spec === $Failed, $Failed,
              (Pole[M[hh]] /. (NUHMSSMNoFVHimalayaEFTHiggs /. spec))[[1]]]
          ];

(* calculate Higgs mass and uncertainty estimate *)
CalcNUHMSSMNoFVHimalayaEFTHiggsDMh[a___, (fsSettings | fsSMParameters | fsModelParameters) -> s_List, r___] :=
    CalcNUHMSSMNoFVHimalayaEFTHiggsDMh[a, Sequence @@ s, r];

CalcNUHMSSMNoFVHimalayaEFTHiggsDMh[args__] :=
    Module[{Mh0, Mh, MhYt4L, varyQpole, varyQmatch,
            DMhSM, DMhSUSY,
            MS = MSUSY /. { args }, Mlow = Mt /. { args }},
           If[!NumericQ[Mlow],
              Mlow = Mt /. Options[FSNUHMSSMNoFVHimalayaEFTHiggsOpenHandle]
             ];
           Mh0        = CalcNUHMSSMNoFVHimalayaEFTHiggsMh[-1, 0, 0, args];
           If[Mh0 === $Failed, Return[{$Failed, $Failed}]];
           Mh         = CalcNUHMSSMNoFVHimalayaEFTHiggsMh[3, 0, 0, args];
           If[Mh === $Failed, Return[{ Mh0, $Failed }]];
           MhYt4L     = CalcNUHMSSMNoFVHimalayaEFTHiggsMh[4, 0, 0, args];
           If[MhYt4L === $Failed, Return[{ Mh0, $Failed }]];
           varyQpole  = CalcNUHMSSMNoFVHimalayaEFTHiggsMh[3, #, 0, args]& /@
                        LogRange[Mlow/2, 2 Mlow, 10];
           If[MemberQ[varyQpole, $Failed], Return[{ Mh0, $Failed }]];
           varyQmatch = CalcNUHMSSMNoFVHimalayaEFTHiggsMh[3, 0, #, args]& /@
