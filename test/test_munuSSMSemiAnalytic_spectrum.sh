...
# This script generates a munuSSM spectrum using both the semi-analytic
# and two-scale solvers, and compares that the results spectrums are
# the same.

BASEDIR=$(dirname $0)

semi_analytic_model="munuSSMSemiAnalytic"
two_scale_model="munuSSM"
...
remove_mixing_and_input_blocks() {
    $awk_cmd -f "$remove_block" -v block=PSEUDOSCALARMIX \
        ...
        | $awk_cmd -f "$remove_block" -v block=YVIN \
