...
B22[p2_, m12_, m22_, q2_, eps_:EPS] :=
    Module[{a01, a02, b0},
           a01 = A0[m12, q2];
           a02 = A0[m22, q2];
           b0 = B0[p2, m12, m22, q2, eps];
           1/6 (
               + 1/2 (a01 + a02)
               + (m12 + m22 - p2/2) b0
               + (m22 - m12) / (2 p2) (a02 - a01 - (m22 - m12) b0)
               + m12 + m22 - p2/3
           )
    ]
...
(*************************************************************
   B22
   Format: p^2, m1^2, m2^2, q^2, B22[p^2, m1^2, m2^2, q^2]
 *************************************************************)

MakePointB22[p2_, m12_, m22_, q2_] :=
    { p2, m12, m22, q2, B22[p2, m12, m22, q2] }

data = ParallelMap[N[MakePointB22 @@ #, digits]&,
    Join[
        Flatten[#,2]& @ Table[{10^n, 10^k, 10^l, 1}, {n, -15, 15},
                              {k, -15, 15}, {l, -15, 15}],
        Flatten[#,5]& @ Table[{1 + s1 10^n, 1 + s2 10^k, 1 + s3 10^l, 1},
