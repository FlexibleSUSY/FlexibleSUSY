...
void FlexibleDecay_settings::set(Settings o, double value)
{
   ...
   case calculate_decays: // 0 [bool]
      ...
      break;
   case higher_orders_in_decays: // 1 [int >= 0 and <= 1]
...
std::ostream& operator<<(std::ostream& ostr, const FlexibleDecay_settings& sgs)
{
   ostr << "(";

   for (int i = 0; i < FlexibleDecay_settings::NUMBER_OF_OPTIONS; i++) {
      ostr << sgs.get(static_cast<FlexibleDecay_settings::Settings>(i));
      if (i < FlexibleDecay_settings::NUMBER_OF_OPTIONS - 1)
         ostr << ", ";
   }
