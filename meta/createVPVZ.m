...
AddLoopVectorDiff[particles_, corrections_] := 
  Block[{particle1, particle2, temp, m1, m2, m0, mS, mV, v, fac, m11, 
   ...
   If[getType[corrections[[1]]] === F, 
    ...
    coup1 = corrections[[3]];
      coup2 = corrections[[4]];
      PrintDebug["Creating corrections for ",particles,corrections];
    Switch[corrections[[5]],
     ...
     pi1Loop = pi1Loop*corrections[[7]]*corrections[[6]];
     (* For compatibility with SPhenoLoopMasses/WriteVectorScalarMixedSE rather than loopCorrections.m If[conj[particle1] === particle1 && conj[particle2] === particle2, pi1Loop = 2 pi1Loop];*)
     Return[sum[gI1, 1, getGen[corrections[[1]]],sum[gI2, 1, getGen[corrections[[2]]], pi1Loop]]];,
     VVV,
     pi1Loop = 10 B00[p^2, m12, m22] + (m12 + m22 + 4*p^2)*B0[p^2, m12, m22] + A0[m12] + A0[m22] - 2 (m12 + m22 - 1/3 p^2)*rMS;
	   (*For compatibility with SPhenoLoopMasses/WriteVectorScalarMixedSE rather than loopCorrections.m  pi1Loop = -pi1Loop*corrections[[7]]*corrections[[6]] coup1 coup2;*)
     	pi1Loop = pi1Loop*corrections[[7]]*corrections[[6]] coup1 coup2;   
